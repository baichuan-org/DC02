...
esp_err_t wifi_scan(wifi_ap_record_simple_t *ap_records, uint16_t *ap_count, uint16_t max_ap_count)
{
    if (is_scanning) {
...
static void event_handler(void * arg, esp_event_base_t event_base, int32_t event_id, void * event_data)
{
    if (event_base == WIFI_EVENT)
    {
        ...
        if (event_id == WIFI_EVENT_STA_START) {
            ...
            if(s_retry_num >= 3 && WIFI_REASON_NO_AP_FOUND == event->reason){
                ESP_LOGI(TAG, "Stop retrying to connect to %s", event->ssid);
...
void generate_ssid(char * ssid)
{
    ...
    snprintf(ssid, 32, "HAMMER_%02X%02X", mac[4], mac[5]);
}
...
void wifi_only_softap(void)
{
    ESP_LOGI(TAG, "ESP_WIFI Only Access Point On");
