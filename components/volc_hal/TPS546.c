...
#define I2C_MASTER_NUM_OF_TPS546 1 /*!< I2C master i2c port number, the number of i2c peripheral interfaces available will depend on the chip */
...
{
    return volc_i2c_register_read(tps546_i2c_handle, command, data, 1);
}
...
{
    return volc_i2c_register_write_byte(tps546_i2c_handle, command, data);
}
...
{
    return volc_i2c_register_write_addr(tps546_i2c_handle, command);
}
...
{
    uint8_t data[2];
    if (volc_i2c_register_read(tps546_i2c_handle, command, data, 2) != ESP_OK) {
        return ESP_FAIL;
    } else {
...
{
    return volc_i2c_register_write_word(tps546_i2c_handle, command, data);
}
...
static esp_err_t smb_read_block(uint8_t command, uint8_t *data, uint8_t len)
{
    ...
    if (volc_i2c_register_read(tps546_i2c_handle, command, buf, len+1) != ESP_OK) {
        free(buf);
        return ESP_FAIL;
    }
...
//     //write it all
//     if (volc_i2c_register_write_bytes(tps546_i2c_handle, buf, len+2) != ESP_OK) {
//         free(buf);
//         return ESP_FAIL;
//     } else {
//         free(buf);
...
/**
 * @brief Set up the TPS546
*/
esp_err_t TPS546_init_test(TPS546_CONFIG config)
{
	uint8_t data[7] = {0x0};
    uint8_t u8_value;
    ...
    ESP_RETURN_ON_ERROR(volc_i2c_add_device(I2C_MASTER_NUM_OF_TPS546, TPS546_I2CADDR, &tps546_i2c_handle, TAG), TAG, "Failed to add TPS546 I2C");
    ...
    if(ret != ESP_OK)
    {
        ...
        while(read_num--)
        {
            ...
            if(read_num == 90)
            {
                extern void show_mining_screen(void);
                show_mining_screen();
            }
            if(smb_read_block(PMBUS_IC_DEVICE_ID, data, 6) == ESP_OK)
            {
                break;
            }
        }
    ...
    {
        smb_write_byte(PMBUS_ON_OFF_CONFIG, 0x1e);
        ESP_LOGI(TAG, "Setting ON_OFF_CONFIG, 0x1e ");

        smb_write_byte(PMBUS_OPERATION, OPERATION_OFF);
        ESP_LOGI(TAG, "Setting OPERATION, 00 ");
        vTaskDelay(pdMS_TO_TICKS(100));

        ESP_LOGI(TAG, "Setting PMBUS_PIN_DETECT_OVERRIDE 0000 ");
        smb_write_word(PMBUS_PIN_DETECT_OVERRIDE, 0);

        ESP_LOGI(TAG, "PMBUS_STORE_USER_ALL");
        smb_write_addr(PMBUS_STORE_USER_ALL);
    ...
    /* Show temperature */
    ESP_LOGI(TAG, "--------------------------------");
    ESP_LOGI(TAG, "read Temp: %d", TPS546_get_temperature());
    ...
    if(u16_value & 0x8000)
    {
        smb_read_word(PMBUS_STATUS_VOUT, &u16_value);
        ESP_LOGI(TAG, "read STATUS_VOUT: %04x", u16_value);
    }
...
esp_err_t TPS546_init(TPS546_CONFIG config)
{
...
#if 0//CONFIG_TPS546_VOUT_MAX > 200
    smb_write_byte(PMBUS_ON_OFF_CONFIG, 0x1f);
    ESP_LOGI(TAG, "Setting ON_OFF_CONFIG, 0x1f ");
    ...
    ESP_LOGI(TAG, "Setting VOUT SCALE: %.3f", tps546_config.TPS546_INIT_SCALE_LOOP);
    ...
    ESP_LOGI(TAG, "Setting OPERATION, 80 ");
    vTaskDelay(pdMS_TO_TICKS(20));

#endif

    #if 0
    smb_write_addr(PMBUS_RESTORE_USER_ALL);
    ESP_LOGI(TAG, "PMBUS_RESTORE_USER_ALL");
    vTaskDelay(pdMS_TO_TICKS(100));
    #endif

    if(tps546_config.TPS546_INIT_VOUT_MAX > 1.5)
    {
        smb_write_byte(PMBUS_ON_OFF_CONFIG, 0x1f);
        ESP_LOGI(TAG, "Setting ON_OFF_CONFIG, 0x1f ");
...
static esp_err_t TPS546_parse_status(uint16_t status) {
    ...
    if (status & TPS546_STATUS_IOUT) {
        ...
        if (smb_read_byte(PMBUS_STATUS_IOUT, &u8_value) != ESP_OK) {
            ...
            ESP_LOGE(TAG, "TPS546 IOUT Status: %02X", u8_value);
...
void TPS546_show_voltage_settings(void)
{
    ...
    ESP_LOGI(TAG, "read VOUT_MODE: %02X", u8_value);
    ...
    /* VOUT_SCALE_LOOP */
    smb_read_word(PMBUS_VOUT_SCALE_LOOP, &u16_value);
    f_value = slinear11_2_float(u16_value);
    ESP_LOGI(TAG, "read VOUT_SCALE_LOOP: %.3f", f_value);
    tps546_config.TPS546_INIT_SCALE_LOOP = f_value;
    ...
    f_value = ulinear16_2_float(u16_value);
    ESP_LOGI(TAG, "read VOUT_MIN: %.2fV", f_value);

    /* VOUT_OV_FAULT_LIMIT */
    smb_read_word(PMBUS_VOUT_OV_FAULT_LIMIT, &u16_value);
    f_value = ulinear16_2_float(u16_value);
    ESP_LOGI(TAG, "read VOUT_OV_FAULT_LIMIT: %.2f", f_value);

    /* VOUT_OV_WARN_LIMIT */
    smb_read_word(PMBUS_VOUT_OV_WARN_LIMIT, &u16_value);
    f_value = ulinear16_2_float(u16_value);
    ESP_LOGI(TAG, "read VOUT_OV_WARN_LIMIT: %.2f", f_value);

    /* VOUT_MARGIN_HIGH */
    smb_read_word(PMBUS_VOUT_MARGIN_HIGH, &u16_value);
    f_value = ulinear16_2_float(u16_value);
    ESP_LOGI(TAG, "read VOUT_MARGIN_HIGH: %.2f", f_value);

    /* VOUT_MARGIN_LOW */
    smb_read_word(PMBUS_VOUT_MARGIN_LOW, &u16_value);
    f_value = ulinear16_2_float(u16_value);
    ESP_LOGI(TAG, "read VOUT_MARGIN_LOW: %.2f", f_value);

    /* VOUT_UV_WARN_LIMIT */
    smb_read_word(PMBUS_VOUT_UV_WARN_LIMIT, &u16_value);
    f_value = ulinear16_2_float(u16_value);
    ESP_LOGI(TAG, "read VOUT_UV_WARN_LIMIT: %.2f", f_value);

    /* VOUT_UV_FAULT_LIMIT */
    smb_read_word(PMBUS_VOUT_UV_FAULT_LIMIT, &u16_value);
    f_value = ulinear16_2_float(u16_value);
    ESP_LOGI(TAG, "read VOUT_UV_FAULT_LIMIT: %.2f", f_value);

    /* PMBUS_IOUT_OC_WARN_LIMIT */
    smb_read_word(PMBUS_IOUT_OC_WARN_LIMIT, &u16_value);
    f_value = slinear11_2_float(u16_value);
    ESP_LOGI(TAG, "read PMBUS_IOUT_OC_WARN_LIMIT: %.2fA", f_value);

    /* PMBUS_IOUT_OC_FAULT_LIMIT */
    smb_read_word(PMBUS_IOUT_OC_FAULT_LIMIT, &u16_value);
    f_value = slinear11_2_float(u16_value);
    ESP_LOGI(TAG, "read PMBUS_IOUT_OC_FAULT_LIMIT: %.2fA", f_value);
}

#define GAMMA_POWER_OFFSET 5
// (float vin, float iin, float pin, float vout, float iout, float pout)
void TPS546_get_pwoer_info(float *pwr)
{
    *pwr++ = TPS546_get_vin();
    *pwr++ = TPS546_get_iout();
    *pwr++ = (TPS546_get_vout() * TPS546_get_iout()) + GAMMA_POWER_OFFSET;
    *pwr++ = TPS546_get_vout();
    *pwr++ = TPS546_get_iout();
    *pwr++ = (TPS546_get_vout() * TPS546_get_iout()) + GAMMA_POWER_OFFSET;
