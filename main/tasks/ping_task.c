...
#define PING_COUNT 4           // number of pings per round
...
PingResult perform_ping(const char* ip_str, const char* hostname_str)
{
    ...
    PingStats stats = { .min_rtt = 1e6, .max_rtt = 0.0, .hostname = hostname_str, .header_shown = false };
...
void ping_task(void *pvParameters) {
    const void* last_config = NULL;
    GlobalState *module = (GlobalState *) pvParameters;

    while (true) {
        if (!stratum_connected_status()) {
            ESP_LOGW(TAG, "Stratum not connected. Skipping ping...");
            vTaskDelay(pdMS_TO_TICKS(10000));
            continue;
        }

        const char* hostname = module->SYSTEM_MODULE.is_using_fallback ? module->SYSTEM_MODULE.fallback_pool_url : module->SYSTEM_MODULE.pool_url;
        const char* ip_str = stratum_get_pool_ip();
