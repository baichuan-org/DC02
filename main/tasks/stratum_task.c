...
void stratum_primary_heartbeat(void * pvParameters)
{
    ...
    while (1)
    {
        ...
        dest_addr.sin_addr.s_addr = inet_addr(STATUM_MODULE.lastResolvedIp);
        ...
        if (err != 0)
        {
            ESP_LOGD(TAG, "Heartbeat. Failed connect check: %s:%d (errno %d: %s)", STATUM_MODULE.lastResolvedIp, primary_stratum_port, errno, strerror(errno));
            close(sock);
            vTaskDelay(60000 / portTICK_PERIOD_MS);
            continue;
        }
...
void stratum_task(void * pvParameters)
{
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "Connecting to: stratum+tcp://%s:%d (%s)", stratum_url, port, STATUM_MODULE.lastResolvedIp);
        ...
        ESP_LOGI(TAG, "Socket created, connecting to %s:%d", STATUM_MODULE.lastResolvedIp, port);
        ...
        /*skip the configure flow of btc.*/
        ...
        while (1) {
            ...
            if (stratum_api_v1_message.method == MINING_NOTIFY) {
                ...
                if (stratum_api_v1_message.new_difficulty != STATUM_MODULE.stratum_difficulty) {
                    ...
                    ESP_LOGD(TAG, "Set stratum difficulty: %ld", STATUM_MODULE.stratum_difficulty);
                    #ifdef STATISTIC_SYSTEM_FEATURE
                ...
                ESP_LOGI(TAG, "Subscribe Reasult: %s, %u", 
